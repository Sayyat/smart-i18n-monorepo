 ...
 *   1) `smart-i18next-cli clean-translations`
 *   2) `smart-i18next-cli` (to regenerate namespaces and types)
 *
...
const warnStaleNamespace = (ns: string, chosen: TKnownNamespace) => {
	if (process.env.NODE_ENV !== "production") {
		console.warn(
			`[smart-i18next-cli] Unknown namespace "${ns}". Using "${chosen}" instead. ` +
			`Tip: run "smart-i18next-cli clean-translations" then "smart-i18next-cli" to remove stale files and refresh types.`
		);
	}
};
